...
, mSelectProjectedJoints(false)
{
	...
	addParameter("Projected coordinates", &mSelectProjectedJoints, "Enable this to output 3D points of the joint positions projected onto the depth image coordinate space (x and y are pixels, z is distance).");
	setupOutlets();
}
...
void NodeSelectJoint::step()
{
	...
	switch (data.type())
	{
		case SKELETON3D:
		{
			...
			{
				Data data = mSelectProjectedJoints? s.jointProjective(mSelectedJoints[i]) : s.joint(mSelectedJoints[i]);
		...
		case SCENE3D:
		{
			...
			for (int i=0; i<mNumOutlets; i++)
			{
				...
				{
					out.value.push_back(mSelectProjectedJoints? skel.jointProjective(mSelectedJoints[i]) : skel.joint(mSelectedJoints[i]));
				}
				Data data(out);
				outlet(i)->outputNewData(data);
			}
			...
			break;
		//	// TODO: Removed these as they look buggy. We should not index into value with a joint id
		//	// as value's indices may not align with joint IDs.
		//case LIST_POINT2D:
		//{
