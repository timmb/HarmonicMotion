...
namespace hm
{
	...
	public:
		...
		explicit
		List(double timestamp=0., int id = BaseData::sDefaultId, SceneMetaPtr sceneMeta=SceneMeta::defaultSceneMeta())
		: BaseType(timestamp, id, sceneMeta)
		{}
		
		List(std::vector<DataType> const& values, double timestamp=0., int id = BaseData::sDefaultId, SceneMetaPtr sceneMeta = SceneMeta::defaultSceneMeta())
		: BaseType(timestamp, id, sceneMeta)
		, value(values)
		{}
